...
connections starts with a call to the ctor function of the TLS client
 (#flea_tls_clt_ctx_t) or server (#flea_tls_srv_ctx_t) context object.
...
- <code>THR_flea_tls_clt_ctx_t__read_app_data()</code>
- <code>THR_flea_tls_clt_ctx_t__send_app_data()</code>
- <code>THR_flea_tls_srv_ctx_t__read_app_data()</code>
- <code>THR_flea_tls_srv_ctx_t__send_app_data()</code>

Note that in order to ensure the sending of the application over the wire a call
to
- <code>THR_flea_tls_clt_ctx_t__flush_write_app_data()</code> or
- <code>#THR_flea_tls_srv_ctx_t__flush_write_app_data()</code>
...
- <code>THR_flea_tls_clt_ctx_t__renegotiate()</code> or
- <code>THR_flea_tls_srv_ctx_t__renegotiate()</code>.

Before a call one of these functions #flea_tls_clt_ctx_t__is_reneg_allowed or #flea_tls_srv_ctx_t__is_reneg_allowed() should be made in order to determine whether renegotiation is possible according to the current configuration.
...
Furthermore, a renegotiation can be executed during a call to a <code>read_app_data</code> function, if the peer initiates a
  ...
  For the TLS server, session resumption is enabled by providing a non-null pointer to a constructed #flea_tls_session_mngr_t  object in the call to #THR_flea_tls_srv_ctx_t__ctor(). The configuration property #FLEA_TLS_MAX_NB_MNGD_SESSIONS determines how many sessions can be cached.


  \subsection secTlsSessResClient TLS Client
For the client, a session can be saved for later resumption by creating an object of type #flea_tls_clt_session_t using the function #flea_tls_clt_session_t__ctor() and providing it in the call to #THR_flea_tls_clt_ctx_t__ctor().
After the successful completion of #THR_flea_tls_clt_ctx_t__ctor(), the #flea_tls_clt_session_t object should now contain a valid session. This can be tested with the function #flea_tls_clt_session_t__has_valid_session(). The valid session can be serialized using #THR_flea_tls_clt_session_t__serialize().

In order to resume a session, first an object of type #flea_tls_clt_session_t containing a valid session has to be present. This can be achieved either

- by keeping the object after the above described procedure to record a session during a handshake,
- or by creating a new #flea_tls_clt_session_t via #flea_tls_clt_session_t__ctor() and then deserializing a previously serialized session using the function #THR_flea_tls_clt_session_t__deserialize() on the same object.

Afterwards, this object is provided to the call to #THR_flea_tls_clt_ctx_t__ctor(), causing the client to attempt to resume that session.
  ...
  closed if an error occurs during any of the other functions of #flea_tls_clt_ctx_t or #flea_tls_srv_ctx_t.
...
#flea_tls_srv_ctx_t and #flea_tls_clt_ctx_t. Their life cycle is modelled such that their
...
Note that for both the client and the server it must be ensured that all pointers provided in the respective ctor call must remain valid throughout the whole life-time of the #flea_tls_clt_ctx_t or #flea_tls_srv_ctx_t object, i.e. until the dtor function is called.
...
The directory contains also a script to start a fleaTLS server against which the client example can run. The script must be invoked from the flea main directory. The TLS client connects to the server and executes a handshake with hostname verification (in this case using the IP address 127.0.0.1) and verifies the authenticity of the server certificate.  Afterwards, it sends some application data which is pinged back by the server. The example does not use client authentication. In order to enable client authentication, #THR_flea_tls_clt_ctx_t__ctor() must be invoked with a certificate chain and a private key in the same way as it is shown for the TLS server in Section \ref secInstTlsServer.
...
parameter in the THR_flea_tls_srv_ctx_t__ctor() must be non-null and contain
