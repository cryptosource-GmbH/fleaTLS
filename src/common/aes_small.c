...
static void flea_aes_small_inv_mix_col (flea_u8_t* state__pu8)
{
  ...
  for(i = 0; i < 4; ++i)
  {
    ...
    for(j = 0; j < 4; j++)
    {
      state__pu8[i * 4 + j] = flea_small_aes_multiply(a, vals__au8[j]) ^ flea_small_aes_multiply(b, vals__au8[(j + 3) % 4]) ^ flea_small_aes_multiply(c, vals__au8[(j + 2) % 4]) ^ flea_small_aes_multiply(d, vals__au8[(j + 1) % 4]);
    }
  }
}
