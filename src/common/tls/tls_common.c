...
static const error_alert_pair_t error_alert_map__act [] = {
  ...
  {FLEA_ERR_PUBKEY_SEC_LEV_NOT_MET,             FLEA_TLS_ALERT_DESC_HANDSHAKE_FAILURE   }
};
...
flea_err_e THR_flea_tls_ctx_t__set_max_fragm_len(
  ...
  flea_al_u8_t              max_fragment_code__alu8
)
{
  ...
  if(FLEA_TLS_RECORD_MAX_RECEIVE_PLAINTEXT_SIZE >= max_fragment_length__alu16)
  {
...
# ifdef FLEA_HEAP_MODE
      // resize recv buffer
      FLEA_CCALL(
        THR_flea_recprot_t__resize_send_plaintext_size(
          &tls_ctx__pt->rec_prot__t,
          max_fragment_length__alu16
        )
      );
...
)
{
  ...
  if(err__t)
  {
    ...
    if(do_send_alert__b)
    {
      ...
      FLEA_CCALL(THR_flea_recprot_t__send_alert_and_throw(&tls_ctx__pt->rec_prot__t, alert_desc__e, err__t));
    }
  }
  FLEA_THR_FIN_SEC_empty();
...
)
{
  ...
  FLEA_CCALL(THR_flea_tls__snd_hands_msg_int_be(hs_ctx__pt, p_hash_ctx, cert_list_len__u32, 3));
...
# endif
  ...
  flea_bool_t allow_dtls__b = FLEA_FALSE;

  FLEA_THR_BEG_FUNC();

# ifdef FLEA_HAVE_DTLS
// TODO: MAKE SUPPR WINDOW SIZE PART OF THE DTLS CONFIGURATION
// IT SHOULD NOT BE DYNAMICALY ADAPTED AFTER RECV-TMOS: IN CASE OF AN ALREADY ENLARGED TMO, A TENDENCY TO MORE RETRANSMISSIONS IS A GOOD IDEA
...
)
{
  ...
  FLEA_CCALL(THR_flea_tls_prl_hash_ctx_t__final(p_hash_ctx, hash_id__t, FLEA_TRUE, messages_hash__pu8));
...
{
  return THR_flea_recprot_t__write_flush(&tls_ctx->rec_prot__t);
}

static flea_err_e THR_flea_tls_ctx_t__snd_appdat_inner(
  flea_tls_ctx_t*  tls_ctx,
...
)
{
  ...
  FLEA_CCALL(THR_flea_recprot_t__write_flush(&tls_ctx->rec_prot__t));
...
)
{
  ...
  err__t      = THR_flea_tls_ctx_t__snd_appdat_inner(tls_ctx__pt, data__pcu8, data_len__dtl);
 ...
 */
static flea_err_e THR_flea_tls_ctx_t__rd_appdat_inner(
  flea_tls_srv_ctx_t*             server_ctx_mbn__pt,
  flea_tls_clt_ctx_t*             client_ctx_mbn__pt,
  flea_u8_t*                      data__pu8,
...
# ifdef FLEA_HAVE_TLS_SERVER
      ...
      {
        flea_recprot_t__discard_current_read_record(&tls_ctx__pt->rec_prot__t);
        FLEA_CCALL(
...
# endif /* ifdef FLEA_HAVE_TLS_CLIENT */
  ...
  FLEA_THR_FIN_SEC_empty();
} /* THR_flea_tls_ctx_t__rd_appdat_inner */
...
)
{
  ...
  err__t = THR_flea_tls_ctx_t__rd_appdat_inner(
    server_ctx_mbn__pt,
    client_ctx_mbn__pt,
    data__pu8,
    data_len__pdtl,
...
flea_al_u16_t flea_tls_ctx_t__compute_extensions_length(flea_tls_ctx_t* tls_ctx__pt)
{
  ...
  if(flea_tls_ctx_t__do_send_sec_reneg_ext(tls_ctx__pt))
  {
    ...
    {
      if(tls_ctx__pt->sec_reneg_flag__u8 && flea_recprot_t__have_done_initial_handshake(&tls_ctx__pt->rec_prot__t))
      {
...
)
{
  ...
  FLEA_THR_BEG_FUNC();
  len__alu16 = flea_tls_ctx_t__compute_extensions_length(hs_ctx__pt->tls_ctx__pt);
...
)
{
  ...
  FLEA_CCALL(THR_flea_tls__snd_hands_msg_content(hs_ctx__pt, p_hash_ctx__pt, &len__u8, 1));
  if(tls_ctx__pt->sec_reneg_flag__u8)
  {
    if(tls_ctx__pt->connection_end == FLEA_TLS_CLIENT)
    {
    ...
    else if(flea_recprot_t__have_done_initial_handshake(&tls_ctx__pt->rec_prot__t))
    {
      len__u8 = 24;
    }
  }
...
)
{
  ...
  if(flea_recprot_t__have_done_initial_handshake(&tls_ctx__pt->rec_prot__t) && !len__u8)
  {
...
)
{
  ...
  if(tls_ctx__pt->connection_end == FLEA_TLS_CLIENT)
  {
    ext_byte__u8 = flea_tls__get_max_fragment_length_byte_for_buf_size(FLEA_TLS_RECORD_MAX_RECEIVE_PLAINTEXT_SIZE);
  }
  else
  {
    flea_tls_srv_ctx_t* server_ctx__pt = (flea_tls_srv_ctx_t*) tls_ctx__pt->client_or_server_ctx__pv;
...
flea_u8_t flea_tls__get_max_fragment_length_byte_for_buf_size(flea_u16_t buf_len__u16)
{
  ...
  if(buf_len__u16 >= 4096 && buf_len__u16 < 16384)
  {
    result = 4;
  }
...
)
{
  ...
  if(ext_len__alu16 != 1)
  {
    FLEA_THROW("invalid size of max_fragment_length extension", FLEA_ERR_TLS_ILLEGAL_PARAMETER);
  }
   ...
   * correct value. If so, the server accepted the negotiation of the fragment
   * length.
...
# ifdef FLEA_TLS_HAVE_MAX_FRAG_LEN_EXT
  if(receive_max_frag_len_ext__b == FLEA_FALSE &&
    flea_tls__get_max_fragment_length_byte_for_buf_size(FLEA_TLS_RECORD_MAX_RECEIVE_PLAINTEXT_SIZE) &&
    tls_ctx__pt->connection_end == FLEA_TLS_SERVER)
  {
    // for sess res case the client can ommit the extension
    if(!hs_ctx__pt->is_sess_res__b)
    {
      FLEA_THROW(
...
# endif /* ifdef FLEA_TLS_HAVE_MAX_FRAG_LEN_EXT */
  ...
  // no signature_algorithms ext. received from client in initial handshake
  if(receive_sig_algs_ext__b == FLEA_FALSE && tls_ctx__pt->connection_end == FLEA_TLS_SERVER &&
    !hs_ctx__pt->is_sess_res__b)
  {
