...
static flea_err_e THR_flea_start_tls_client(
  ...
  flea_tls_clt_session_t* client_session__pt
)
{
  ...
  {
    FLEA_CCALL(
      THR_flea_privkey_t__ctor_pkcs8(
        &privkey__t,
...
#  endif // ifdef FLEA_HAVE_TLS_CS_PSK
  ...
  {
    int reneg_allowed = flea_tls_clt_ctx_t__is_reneg_allowed(&tls_ctx);
    ...
    FLEA_CCALL(
      THR_flea_tls_clt_ctx_t__renegotiate(
        &tls_ctx,
        &reneg_done__b,
        &trust_store__t,
        cert_chain,
  ...
  while(cmdl_args.have_index("stay"))
  {
    ...
    flea_err_e retval =
      THR_flea_tls_clt_ctx_t__read_app_data(&tls_ctx, buf, &buf_len, tls_cfg.read_mode_for_app_data);
    ...
    FLEA_CCALL(THR_flea_tls_clt_ctx_t__send_app_data(&tls_ctx, buf, buf_len));
    FLEA_CCALL(THR_flea_tls_clt_ctx_t__flush_write_app_data(&tls_ctx));
    buf[buf_len] = 0;
...
int flea_start_tls_client(property_set_t const& cmdl_args)
{
  ...
  flea_tls_clt_session_t client_session__t;
  int retval = 0;

  FLEA_DECL_flea_byte_vec_t__CONSTR_HEAP_ALLOCATABLE_OR_STACK(serialized_session_t, 200);
  flea_tls_clt_session_t__INIT(&client_session__t);

  flea_tls_clt_session_t__ctor(&client_session__t);
  if(flea_tls_clt_session_t__has_valid_session(&client_session__t))
  {
    throw test_utils_exceptn_t("internal error with flea_tls_clt_session_t object");
  }
  ...
  else if(cmdl_args.have_index("session") || cmdl_args.have_index("session_in"))
  {
    ...
    err = THR_flea_tls_clt_session_t__deserialize(&client_session__t, &sid[0], sid.size());
  }
  ...
  else
  {
    ...
    if(flea_tls_clt_session_t__has_valid_session(&client_session__t))
    {
      err = THR_flea_tls_clt_session_t__serialize(&client_session__t, &serialized_session_t);
      if(err)
      {
  ...
  flea_tls_clt_session_t__dtor(&client_session__t);
