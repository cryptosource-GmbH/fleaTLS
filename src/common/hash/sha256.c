...
)
{
  ...
  // Compress
# ifdef FLEA_USE_SHA256_LOOP_UNROLL
  for(i = 0; i < 64; i += 8)
  {
    flea_u32_t t0;
    flea_u32_t t1;
...
# else  /* ifdef FLEA_USE_SHA256_LOOP_UNROLL */
  for(i = 0; i < 64; i++)
  {
    flea_u32_t t0;
    flea_u32_t t1;
...
# endif /* ifdef FLEA_USE_SHA256_LOOP_UNROLL */
...
{
  flea_al_u8_t i;
  const flea_u32_t* state = ctx__t->hash_state;
