...
static flea_err_t THR_flea_test_ecdsa_raw_basic_inner(const flea_ec_gfp_dom_par_ref_t *dom_par__pt)
{
  ...
  FLEA_CCALL(THR_flea_generate_ecc_key(pub_point_enc__b_u8, &pub_point_enc_len__al_u8, sk_enc__b_u8, &sk_enc_len__al_u8, dom_par__pt));


  // ...encode x and y of pub point and input them to verify
  for(i = 0; i < 1; i++)
  {
    FLEA_CCALL(THR_flea_ecdsa__raw_sign(res_r_arr, &enc_r_len, res_s_arr, &enc_s_len, message, sizeof(message), sk_enc__b_u8, sk_enc_len__al_u8, dom_par__pt));
    FLEA_CCALL(THR_flea_ecdsa__raw_verify(res_r_arr, enc_r_len, res_s_arr, enc_s_len, message, sizeof(message), pub_point_enc__b_u8, pub_point_enc_len__al_u8, dom_par__pt));
    res_r_arr[0] ^= 1;
    if(FLEA_ERR_INV_SIGNATURE != THR_flea_ecdsa__raw_verify(res_r_arr, enc_r_len, res_s_arr, enc_s_len, message, sizeof(message), pub_point_enc__b_u8, pub_point_enc_len__al_u8, dom_par__pt))
    {
...
flea_err_t THR_flea_test_ecdsa_256bit_sign_loop(unsigned count)
{
  ...
  FLEA_CCALL(THR_flea_generate_ecc_key(pub_point_enc__b_u8, &pub_point_enc_len__al_u8, sk_enc__b_u8, &sk_enc_len__al_u8, &dom_par__t));


  // ...encode x and y of pub point and input them to verify
  for(i = 0; i < count; i++)
  {
    FLEA_CCALL(THR_flea_ecdsa__raw_sign(res_r_arr, &enc_r_len, res_s_arr, &enc_s_len, message, sizeof(message), sk_enc__b_u8, sk_enc_len__al_u8, &dom_par__t));
  }
  FLEA_THR_FIN_SEC(
    FLEA_FREE_BUF_FINAL(pub_point_enc__b_u8);
    FLEA_FREE_BUF_FINAL(sk_enc__b_u8);
