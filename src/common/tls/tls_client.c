...
)
{
  ...
  hs_ctx__pt->tls_ctx__pt = tls_ctx;
...
# endif /* ifdef FLEA_HEAP_MODE */
  ...
  while(1)
  {
    ...
    {
      // send client hello
      FLEA_CCALL(THR_flea_tls__send_client_hello(tls_ctx, hs_ctx__pt, &p_hash_ctx, session_mbn__pt));
...
)
{
  ...
  hs_ctx__t.is_reneg__b = FLEA_FALSE;
