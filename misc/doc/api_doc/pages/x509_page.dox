...
In the call to #THR_flea_cpv_t__ctor_cert(), the certificate to validate (the target certificate) is provided.
With #THR_flea_cpv_t__add_trust_anchor_cert(), at least one certificate has to be added. The path validation is successful when a valid chain up to one of the trusted certificates can be built.

The function #THR_flea_cpv_t__validate(), carries out the actual verification. It performs an exhaustive path search using the certificates that have been added to the #flea_cpv_t object.

Untrusted certificates can be added using #THR_flea_cpv_t__add_cert_without_trust_status(). During the validation, these certificates are used as intermediate certificates to complete the chain from the target certificate to the trust anchor.

In order to enable revocation checking, during the construction of the #flea_cpv_t the appropriate #flea_rev_chk_mode_e has to be set and afterwards CRLs have to be added using the function #THR_flea_cpv_t__add_crl().
