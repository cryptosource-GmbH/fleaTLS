...
\usepackage[margin=2.0cm,tmargin=2.9cm,bmargin=3.5cm,headsep=1.7cm,footskip=2.5cm]{geometry}
...
\fancyhead[R]{\hspace{-1.9cm}\includegraphics[width=4.5cm]{cs_logo.pdf}}
\fancyhead[L]{
  \includegraphics[width=3cm]{./fleaTLS_Logo_RGB_addon_pure.eps}
}
\fancyhead[C]{
fleaTLS \thisFleaVersion\\manual}
...
\lstset{language=C,
  emph={int,char,double,float,unsigned,flea\_u8\_t,flea\_al\_u8\_t,flea\_u16\_t,flea\_al\_u16\_t,flea\_u32\_t,
  flea\_bool\_t, flea\_mutex\_func\_set\_t}, emphstyle={\color{csblue}},
...
\newcommand{\thisFleaVersion}{1.3\xspace }
...
\author{cryptosource GmbH,
  Darmstadt\\\url{flea@cryptosource.de}\\\url{www.cryptosource.de}\vspace{0.5cm}\\\includegraphics[width=7cm]{./cs_logo.pdf}}
...
compiles and runs also on 64-bit machines.
  ...
  a specific encoding. Only \code{ const char* } function parameters are expected to be
  null-terminated, character strings which are held in byte arrays such as
  \code{flea\_u8\_t*} or in a \code{flea\_byte\_vec\_t} may not be
...
\begin{comment}
This allows for greater flexibility and security for a number of reasons:
\begin{itemize}
...
\code{THR\_}. Such a function returns \code{FLEA\_ERR\_FINE}, which is defined as
...
used within the function must have be initialized. 
  %  If only the \verb#FLEA\_DECL\_OBJ()# is used for the
...
switching between stack and heap mode in his own code, may certainly use any explicit heap or stack buffer allocation as he
is used to.
\end{comment}
...
fleaTLS features support for random number generation. On the one hand, there is the type \code{flea\_ctr\_mode\_prng\_t}, defined in the file
...
\begin{itemize}
  \item \code{THR\_flea\_tls\_client\_ctx\_t\_\_read\_app\_data()}
\item \code{THR\_flea\_tls\_client\_ctx\_t\_\_send\_app\_data()}
\item \code{THR\_flea\_tls\_server\_ctx\_t\_\_read\_app\_data()}
  \item \code{THR\_flea\_tls\_server\_ctx\_t\_\_send\_app\_data()}
\end{itemize}
...
\begin{itemize}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_128\_CBC\_SHA}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_128\_CBC\_SHA256}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_128\_GCM\_SHA256}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_256\_CBC\_SHA}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_256\_CBC\_SHA384}
\item\code{TLS\_ECDHE\_ECDSA\_WITH\_AES\_256\_GCM\_SHA384}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_128\_CBC\_SHA}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_128\_CBC\_SHA256}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_128\_GCM\_SHA256}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_256\_CBC\_SHA}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_256\_CBC\_SHA384}
\item\code{TLS\_ECDHE\_RSA\_WITH\_AES\_256\_GCM\_SHA384}
\item\code{TLS\_PSK\_WITH\_AES\_128\_CBC\_SHA}
\item\code{TLS\_PSK\_WITH\_AES\_128\_CBC\_SHA256}
\item\code{TLS\_PSK\_WITH\_AES\_128\_GCM\_SHA256}
\item\code{TLS\_PSK\_WITH\_AES\_256\_CBC\_SHA}
\item\code{TLS\_PSK\_WITH\_AES\_256\_CBC\_SHA384}
\item\code{TLS\_PSK\_WITH\_AES\_256\_GCM\_SHA384}
\item\code{TLS\_RSA\_WITH\_AES\_128\_CBC\_SHA}
\item\code{TLS\_RSA\_WITH\_AES\_128\_CBC\_SHA256}
\item\code{TLS\_RSA\_WITH\_AES\_128\_GCM\_SHA256}
\item\code{TLS\_RSA\_WITH\_AES\_256\_CBC\_SHA}
\item\code{TLS\_RSA\_WITH\_AES\_256\_CBC\_SHA256}
\item\code{TLS\_RSA\_WITH\_AES\_256\_GCM\_SHA384}
...
configuration file with the corresponding defines prefixed with \code{FLEA}\ldots .
...
%\todo{add open-ssl command to create certificate with named curve}
...
\code{flea\_tls\_flag\_\_reneg\_mode\_\_disallow\_reneg}, 
\code{flea\_tls\_flag\_\_reneg\_mode\_\_allow\_secure\_reneg}, or 
\code{flea\_tls\_flag\_\_reneg\_mode\_\_allow\_insecure\_reneg}
in argument of type \code{flea\_tls\_flag\_e} in the
...
the parameter of type \code{flea\_tls\_sigalg\_e } of the client or server ctor
function.
...
specifying the parameter of type \code{flea\_ec\_dom\_par\_id\_e} of the client or
...
\code{flea\_tls\_session\-\_mngr\_t} employed, a mutex mechanism needs
...
In the ``multithreading'' section of the file \code{build\_config\_gen.h} 
...
\code{flea\_mutex\_func\_set\_t} must be provided to that function. In this object,
all four member function pointers must be set to point to appropriate functions.
These functions will be called with objects of type \code{FLEA\_MUTEX\_TYPE}
