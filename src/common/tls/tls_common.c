...
# ifdef FLEA_USE_HEAP_BUF
  FLEA_ALLOC_MEM(p_hash_ctx->hash_ctx__pt, hashes_len__u8 * sizeof(flea_hash_ctx_t));
# endif

  p_hash_ctx->num_hash_ctx__u8 = hashes_len__u8;

  for(flea_u8_t i = 0; i < hashes_len__u8; i++)
  {
    FLEA_CCALL(THR_flea_hash_ctx_t__ctor(&p_hash_ctx->hash_ctx__pt[i], hashes__pt[i]));
  }
