...
*
As public-key schemes, fleaTLS supports RSA and elliptic-curve-based schemes through an API for generic public and private key objects, namely #flea_public_key_t and #flea_private_key_t.
...
*
flea_public_key_t and flea_private_key_t objects, however, internally store copies of the referenced domain parameters and thus do not rely on the originally referenced values to remain elsewhere in memory.
...
- #THR_flea_public_key_t__ctor_rsa()
...
fleaTLS currently support only the encoding of public and private key objects in "plain" format by the use of the functions #THR_flea_public_key__t__get_encoded_plain and #THR_flea_private_key_t__get_encoded_plain. Refer to the documentation of these functions regarding the concrete format of the encoded keys.


\subsubsection secPkKeyEccGenKey EC Key Generation

EC keys can be generated using one of the functions #THR_flea_pubkey__generate_ecc_key_pair_by_dp_id() or #THR_flea_pubkey__generate_ecc_key_pair_by_dp().
...
Signatures can be generated using either of the functions #THR_flea_private_key_t__sign() or #THR_flea_private_key_t__sign_digest().
...
Signatures can be verified using either of the functions #THR_flea_public_key_t__verify_signature() or #THR_flea_public_key_t__verify_digest().
...
The encryption is performed using the function. #THR_flea_public_key_t__encrypt_message() and providing either #flea_pkcs1_v1_5 or #flea_oaep as the scheme ID. The hash ID parameter of this function is only used in OAEP, and is not relevant for the PKCS#1 v1.5 scheme.

\subsubsection secPkEncrAlgsDecr Decryption Operation

The function #THR_flea_private_key_t__decrypt_message() is used to decrypt messages. Note that for PKCS#1 v1.5 decryption, rather the use of the function #THR_flea_private_key_t__decrypt_message_secure() is recommended. PKCS#1 v1.5 is a legacy scheme with a security flaw affecting the decryption operation that in many application contexts need to be mitigated. The latter function achieves this by realizing state-of-the-art countermeasures such as constant time decryption.
