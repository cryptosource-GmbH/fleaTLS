...
)
{
  ...
  while(flea_bdec_t__has_current_more_data(&dec__t))
  {
    ...
    FLEA_CCALL(
      THR_flea_bdec_t__op_cons_cft_optional(
        &dec__t,
    ...
    if(!relative_to_issuer__b && full_name_present__b)
    {
      ...
      while(flea_bdec_t__has_current_more_data(&dec__t))
      {
        ...
        FLEA_CCALL(THR_flea_bdec_t__get_r_opl_next_tlv_raw(&dec__t, &raw_tlv__rcu8));
    ...
    if(found_match__b)
    {
      ...
      FLEA_CCALL(
        THR_flea_bdec_t__dec_optl_short_bit_str_to_u32(
          &dec__t,
...
)
{
  ...
  while(flea_bdec_t__has_current_more_data(dec__pt))
  {
    flea_byte_vec_t raw_tlv__rcu8 = flea_byte_vec_t__CONSTR_ZERO_CAPACITY_NOT_ALLOCATABLE;
    /* decode the raw dp name */
    FLEA_CCALL(THR_flea_bdec_t__get_r_opl_next_tlv_raw(dec__pt, &raw_tlv__rcu8));
    FLEA_CCALL(
      THR_flea_crl__does_cdp_contain_distrib_point(
...
)
{
  ...
  else
  {
    ...
    FLEA_CCALL(THR_flea_bdec_t__get_r_opl_next_tlv_raw(dec__pt, &rel_name__rcu8));
    /* CRL-DP must match */
    FLEA_CCALL(
      THR_flea_crl__does_cdp_contain_distrib_point(
        subjects_crldp_raw_mbn__pt,
  ...
  while(flea_bdec_t__has_current_more_data(dec__pt))
  {
    ...
    else if(ext_oid_ref__t.data__pu8[2] == ISSUING_DISTR_POINT_INDIC)
    {
      ...
      if(distrib_point_name_found__b)
      {
        ...
        FLEA_CCALL(THR_flea_bdec_t__cl_cons_at_end(&cont_dec__t));
      }
      FLEA_CCALL(THR_flea_bdec_t__dec_bool_def(&cont_dec__t, &only_contains_user_certs__b));
      FLEA_CCALL(THR_flea_bdec_t__dec_bool_def(&cont_dec__t, &only_contains_ca_certs__b));
      FLEA_CCALL(
        THR_flea_bdec_t__dec_optl_short_bit_str_to_u32(
          &cont_dec__t,
          &only_some_reasons__u32,
      ...
      FLEA_CCALL(THR_flea_bdec_t__dec_bool_def(&cont_dec__t, &indirect_crl__b));
      FLEA_CCALL(THR_flea_bdec_t__dec_bool_def(&cont_dec__t, &only_contains_attrib_certs__b));
...
static flea_err_e THR_flea_crl__by_stream_update_revocation_status_from_crl(
...
)
{
  ...
  FLEA_CCALL(THR_flea_bdec_t__get_r_next_tlv_raw(&dec_tbs__t, &tbs__rcu8));

  FLEA_CCALL(THR_flea_rw_stream_t__ctor_memory(&source__t, crl_der__pcu8, crl_der_len__dtl, &hlp__t));
  FLEA_CCALL(THR_flea_bdec_t__ctor(&dec__t, &source__t, 0, flea_dec_ref));

  FLEA_CCALL(THR_flea_bdec_t__open_sequence(&dec__t)); // crl seq
  FLEA_CCALL(THR_flea_bdec_t__open_sequence(&dec__t)); // tbs seq
  FLEA_CCALL(
    THR_flea_bdec_t__dec_int_u32_default(
      &dec__t,
  ...
  FLEA_CCALL(THR_flea_x509__dec_algid_ref(&algid_ref_1__t, &dec__t));
  ...
  if(have_revoked_certs__b)
  {
    while(flea_bdec_t__has_current_more_data(&dec__t))
    {
      ...
      FLEA_CCALL(THR_flea_bdec_t__open_sequence(&dec__t)); // entry seq
      FLEA_CCALL(THR_flea_bdec_t__dec_int(&dec__t, &serial_number__rcu8));
      ...
      if(have_entry_extensions__b)
      {
        while(flea_bdec_t__has_current_more_data(&dec__t))
        {
          ...
          FLEA_CCALL(THR_flea_bdec_t__open_sequence(&dec__t)); // this entry ext seq
          FLEA_CCALL(THR_flea_bdec_t__get_der_ref_to_oid(&dec__t, &oid__rcu8));
          FLEA_CCALL(THR_flea_bdec_t__dec_bool_def_false(&dec__t, &critical__b));
          if(critical__b)
          {
  ...
  FLEA_CCALL(THR_flea_x509__dec_algid_ref(&algid_ref_2__t, &dec__t));
...
)
{
  FLEA_THR_BEG_FUNC();
  FLEA_CCALL(
    THR_flea_crl__by_stream_update_revocation_status_from_crl(
      crl_der__pcu8,
      crl_der_len__dtl,
