...
static const flea_block_cipher_config_entry_t* flea_find_block_cipher_config(flea_block_cipher_id_e id)
{
  flea_al_u16_t i;
...
flea_err_e THR_flea_ecb_mode_ctx_t__ctor(
  flea_ecb_mode_ctx_t*   p_ctx,
  flea_block_cipher_id_e ext_id__t,
  const flea_u8_t*       key,
  flea_al_u16_t          key_byte_length,
...
)
{
  ...
  for(i = 0; i < nb_blocks_al_u16; i++)
  {
    ...
    flea__increment_encoded_BE_int(
      p_ctx->ctr_block__bu8 + block_length_al_u8 - p_ctx->ctr_len__alu8,
      p_ctx->ctr_len__alu8
    );
...
{
  return THR_flea_cbc_mode__crypt_data(
    id__t,
    key__pcu8,
    key_len__alu8,
    iv__pcu8,
