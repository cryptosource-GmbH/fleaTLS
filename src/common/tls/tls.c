...
typedef struct {
	...
	flea_tls__protocol_version_t version;				/* TLS version */

	flea_u8_t* session_id;							/* Session ID for later resumption */
	...
	CompressionMethod selected_compression_method;	/* negotiated compression method */

	flea_u8_t master_secret[48];					/* Master Secret is always 48 byte long */
	...
	flea_tls__mac_algorithm_t mac_algorithm;		/* information about negotiated mac algorithm */
	flea_u8_t mac_length;							/* length generated by MAC algorithm */
	flea_u8_t mac_key_length;						/* key length for MAC algorithm */
...
*/
flea_err_t read_record_message(flea_tls_ctx_t* tls_ctx, flea_u8_t* bytes, flea_u32_t length, Record* record, RecordType record_type) {
	FLEA_THR_BEG_FUNC();
...
*/
flea_err_t read_server_hello(flea_tls_ctx_t* tls_ctx, HandshakeMessage* handshake_msg, ServerHello* server_hello_msg)
{
	FLEA_THR_BEG_FUNC();
...
ClientHello create_hello_message(flea_tls_ctx_t* tls_ctx)	{
	...
	hello.compression_methods = tls_ctx->compression_methods;
	hello.compression_methods_length = tls_ctx->compression_methods_len;
	memcpy(hello.compression_methods, tls_ctx->compression_methods, tls_ctx->compression_methods_len);
...
void create_record(flea_tls_ctx_t* tls_ctx, Record* record, flea_u8_t* data, flea_u16_t length, ContentType content_type, RecordType record_type) {
	...
	else if (record_type == RECORD_TYPE_CIPHERTEXT)
	{
		/**
...
void flea_tls_ctx_t__ctor(flea_tls_ctx_t* ctx, flea_u8_t* session_id, flea_u8_t session_id_len) {
	/* specify connection end */
	ctx->connection_end = FLEA_TLS_CLIENT;
	...
	ctx->compression_methods = calloc(1, sizeof(flea_u8_t));
	ctx->compression_methods[0] = NO_COMPRESSION;
	ctx->compression_methods_len = 1;
}
...
flea_err_t flea_tls_handshake(int socket_fd)
{
	...
	while(!handshake_initiated)	// TODO: State Machine!
	{
		...
		while (reply_index != recv_bytes)
		{
			...
			else if(handshake_message.type == HANDSHAKE_TYPE_SERVER_HELLO_DONE)
			{
				...
				create_record(&encrypted_finished_record, finished_message_handshake_bytes, finished_message_handshake_bytes_length, CONTENT_TYPE_HANDSHAKE, RECORD_TYPE_CIPHERTEXT);
