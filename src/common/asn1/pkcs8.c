...
# ifdef FLEA_HAVE_RSA
static flea_err_e THR_flea_privkey_t__p8_create_rsa_key(
  flea_privkey_t* privkey_mbn__pt,
...
)
{
  ...
  {
    FLEA_CCALL(THR_flea_bdec_t__get_der_REF_to_positive_int_wo_lead_zeroes(dec__pt, &key_components_arcu8[i]));
  }
  ...
  if(pubkey_mbn__pt)
  {
    FLEA_CCALL(THR_flea_pubkey_t__ctor_rsa(pubkey_mbn__pt, &key_components_arcu8[1], &key_components_arcu8[2]));
  }
  FLEA_THR_FIN_SEC_empty();
} /* THR_flea_privkey_t__p8_create_rsa_key */

# endif /* #ifdef FLEA_HAVE_RSA */

# ifdef FLEA_HAVE_ECC
static flea_err_e THR_flea_privkey_t__p8_create_ecc_key(
  flea_privkey_t*        privkey_mbn__pt,
  flea_pubkey_t*         pubkey_mbn__pt,
  flea_bdec_t*           dec__pt,
...
)
{
   ...
   * }*/
  FLEA_CCALL(THR_flea_bdec_t__rd_val(dec__pt, FLEA_ASN1_INT, 0, &version_vec__t));
  ...
  if(privkey_mbn__pt)
  {
    FLEA_CCALL(THR_flea_privkey_t__ctor_ecc(privkey_mbn__pt, &ostr__t, &dp_ref__t));
  }
  if(pubkey_mbn__pt)
  {
    ...
    FLEA_CCALL(THR_flea_pubkey_t__ctor_ecc(pubkey_mbn__pt, &public_point_encoded__rcu8, &dp_ref__t));
  }
  FLEA_THR_FIN_SEC();
} /* THR_flea_privkey_t__p8_create_ecc_key */
...
# endif
  ...
  FLEA_CCALL(THR_flea_x509__dec_algid_ref(&algid_ref__t, &dec__t));
  FLEA_CCALL(
    THR_flea_bdec_t__get_r_raw_cft(
      &dec__t,
      FLEA_ASN1_CFT_MAKE2(FLEA_ASN1_UNIVERSAL_PRIMITIVE, FLEA_ASN1_OCTET_STRING),
  ...
  FLEA_CCALL(THR_flea_bdec_t__ctor(&cont_dec__t, &cont_source__t, 0, flea_dec_ref));
...
# ifdef FLEA_HAVE_RSA
  ...
  {
    /* RSA key */
    FLEA_CCALL(THR_flea_privkey_t__p8_create_rsa_key(privkey_mbn__pt, pubkey_mbn__pt, &cont_dec__t));
  }
  else
# endif /* ifdef FLEA_HAVE_RSA */
# ifdef FLEA_HAVE_ECC
