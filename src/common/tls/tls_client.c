...
)
{
  ...
  if(psk_identity_hint_len__u32 > FLEA_PSK_MAX_IDENTITY_HINT_LEN)
  {
  ...
  if(!tls_ctx__pt->process_identity_hint_mbn__cb)
  {
    // call the cb function that can use the identity hint to derive the PSK
    tls_ctx__pt->process_identity_hint_mbn__cb(
...
# endif /* ifdef FLEA_HAVE_TLS_CS_ECC */
  ...
  FLEA_CCALL(
    THR_flea_tls__send_handshake_message_int_be(
      &tls_ctx->rec_prot__t,
      p_hash_ctx,
      2 * tls_ctx->nb_allowed_cipher_suites__u16,
      2
    )
  );
  ...
  for(i = 0; i < tls_ctx->nb_allowed_cipher_suites__u16; i++)
  {
    FLEA_CCALL(
      THR_flea_tls__send_handshake_message_int_be(
        &tls_ctx->rec_prot__t,
        p_hash_ctx,
        tls_ctx->allowed_cipher_suites__pe[i],
        2
      )
    );
  }
...
static flea_err_e THR_flea_tls__send_client_key_exchange_rsa(
  flea_tls_ctx_t*               tls_ctx,
  flea_tls_parallel_hash_ctx_t* p_hash_ctx,
    ...
    else
    {
      ...
      if(!session_mbn__pt || !session_mbn__pt->for_resumption__u8)
      {
        ...
        if(session_mbn__pt && session_mbn__pt->session_id_len__u8)
        {
          ...
          session_mbn__pt->session__t.cipher_suite_id__u16 = tls_ctx->selected_cipher_suite__e;
        }
      }
      else
      {
...
)
{
  ...
  tls_client_ctx__pt->tls_ctx__t.process_identity_hint_mbn__cb = process_identity_hint_mbn__cb;
  ...
  FLEA_CCALL(
    THR_flea_tls_client_ctx_t__ctor(
      ...
      NULL,// flea_private_key_t*               private_key_mbn__pt,
      ...
      flags__e,
      NULL // flea_tls_client_session_t*        session_mbn__pt
    )
  );
...
flea_err_e THR_flea_tls_client_ctx_t__flush_write_app_data(flea_tls_client_ctx_t* tls_client_ctx__pt)
{
