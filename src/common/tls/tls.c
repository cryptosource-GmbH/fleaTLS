 ...
 * in the byte stream.
 */
void flea_tls__client_hello_to_bytes(
...
)
{
  ...
  // TO/DO: keine Längenprüfung nötig?
  // set handshake type
  out[i++] = type;
...
void flea_tls__record_to_bytes(
  Record*     record,
  flea_u8_t*  bytes,
  flea_u16_t* length
)
...
void flea_tls__create_hello_message(
  flea_tls_ctx_t*           tls_ctx,
  flea_tls__client_hello_t* hello
)
{
...
flea_err_t THR_flea_tls__receive(
  int         socket_fd,
  flea_u8_t*  buff,
  flea_u32_t  buff_size,
  flea_u32_t* res_len
)
{
  FLEA_THR_BEG_FUNC();
...
flea_err_t THR_flea_tls__send_client_key_exchange_rsa(
  flea_tls_ctx_t*    tls_ctx,
  flea_hash_ctx_t*   hash_ctx,
  flea_public_key_t* pubkey
)
{
  ...
  flea_u32_t premaster_secret_len = 48;
  ...
  // random 46 byte
  flea_rng__randomize(tls_ctx->premaster_secret + 2, premaster_secret_len - 2);
...
)
{
  ...
  FLEA_CCALL(THR_flea_tls__send_client_key_exchange_rsa(tls_ctx, hash_ctx, pubkey));
        ...
        else
        {
          ...
          FLEA_CCALL(
            THR_flea_tls__create_connection_params(
              tls_ctx,
              tls_ctx->pending_read_connection_state,
              &cipher_suites[1],
              FLEA_FALSE
            )
          );
